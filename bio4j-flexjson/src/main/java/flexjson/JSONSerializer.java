 ...
 * {@link flexjson.JSON}. You can include and exclude fields permenantly using
 ...
 * {@link flexjson.transformer.HtmlEncoderTransformer}. Transformers are
 ...
 * {@link JSONSerializer#transform(flexjson.transformer.Transformer, String...)}
 * , or {@link JSONSerializer#exclude(String...)} from multiple threads at the
 * same time. It is also NOT safe to use
 * {@link JSONSerializer#serialize(Object)} and include/exclude/transform from
 * multiple threads. The reason for not making them more thread safe is to boost
 * performance. Typical use case won't call for two threads to modify the
...
public class JSONSerializer {
	...
	private TypeTransformerMap typeTransformerMap = null;
	...
	public JSONSerializer() {
		...
		this.typeTransformerMap = new TypeTransformerMap(TransformerUtil.getDefaultTypeTransformers(this.context));
	...
	protected String serialize(Object target, SerializationType serializationType, OutputHandler out) {
		String output = "";
		// initialize context
		this.context.setRootName(rootName);
		this.context.setPrettyPrint(prettyPrint);
		this.context.setOut(out);
		this.context.serializationType(serializationType);
		this.context.setTypeTransformers(typeTransformerMap);
		this.context.setPathTransformers(pathTransformerMap);
		this.context.setPathExpressions(pathExpressions);

		try {
			// initiate serialization of target tree
			String rootName = this.context.getRootName();
			if (rootName == null || rootName.trim().equals("")) {
				this.context.transform(target);
			} else {
				this.context.writeOpenObject();
				this.context.writeName(rootName);
				this.context.transform(target);
				this.context.writeCloseObject();
			}

			output = this.context.getOut().toString();
		} finally {
			// cleanup context
			// JSONContext.cleanup();
	...
	public JSONSerializer transform(Transformer transformer, String... fields) {
		transformer = new TransformerWrapper(transformer, this.context);
